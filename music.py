...
<script>
    ...
    const translations = {
        ...
        ar: {
            ...
            playPreview: "استمع للأغنية داخل الموقع",
...
<body class="musicy auth-page">
    ...
    <main class="auth-main">
        <div class="auth-art flex flex-col items-center justify-center">
            <img id="profilePreviewImg" src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200" class="avatar-lg">
        ...
        <div class="auth-panel">
            ...
            <form id="profileForm" onsubmit="handleProfileUpdate(event)">
                <div class="field">
                    ...
                    <input type="file" id="avatarFileInput" accept="image/*" class="file mt-2">
                </div>
    ...
    <script>
        ...
        function handleProfileUpdate(e) {
            ...
            if(fileInput.files.length === 0) {
                errDiv.innerText = "يرجى اختيار صورة أولاً";
                errDiv.classList.remove('hidden');
                return;
            }
            ...
            .then(data => {
                if(data.success) {
                    localStorage.setItem('songdb_avatar', data.avatar);
                    document.getElementById('profilePreviewImg').src = data.avatar;
                    successDiv.innerText = "تم تحديث الصورة الشخصية بنجاح!";
...
# صفحة تفاصيل الأغنية مع إضافة مشغل الصوت المدمج (Audio Player)
...
<body class="musicy">
    ...
    <main class="page">
        <section class="track">
            ...
            <div class="track-info">
                ...
                <!-- مشغل الصوت المدمج داخل الموقع -->
                {% if song.preview_url %}
                <div class="mt-6 p-4 rounded-xl border border-theme/30 bg-[#0b0f19]/80 backdrop-blur shadow-lg">
                    <div class="flex items-center gap-3 mb-2 text-xs text-theme font-medium">
                        <i class="fa-solid fa-headphones"></i>
                        <span data-i18n="playPreview">استمع للأغنية داخل الموقع</span>
                    </div>
                    <audio controls class="w-full accent-emerald-500">
                ...
                <div class="track-rating">
                    <div class="big-score" id="songScoreDisplay">{{ song.rating }}</div>
                    <div>
                ...
                <div class="actions">
                    ...
                    <button type="button" onclick="openRateModal()" class="btn btn-ghost"><i class="fa-solid fa-star"></i><span data-i18n="rateSongBtn" id="rateBtnText">Rate This Song</span></button>
                </div>
            </div>
        </section>
                        ...
                        <div class="review" id="review-{{ r.id }}">
                            ...
                            <div class="review-foot">
                                <button type="button" onclick="likeReview({{ r.id }})" class="like-btn" id="like-btn-{{ r.id }}">
                                    ...
                                    <b id="like-count-{{ r.id }}">{{ r.likes }}</b>
                                </button>
                            </div>
                        </div>
                        {% endfor %}
        ...
        <div class="dialog">
            ...
            <div class="field">
                ...
                <div id="starContainer">
                    <i class="fa-solid fa-star" onclick="setRating(1)" onmouseover="hoverRating(1)" onmouseout="resetHover()"></i>
                    <i class="fa-solid fa-star" onclick="setRating(2)" onmouseover="hoverRating(2)" onmouseout="resetHover()"></i>
                    <i class="fa-solid fa-star" onclick="setRating(3)" onmouseover="hoverRating(3)" onmouseout="resetHover()"></i>
                    <i class="fa-solid fa-star" onclick="setRating(4)" onmouseover="hoverRating(4)" onmouseout="resetHover()"></i>
                    <i class="fa-solid fa-star" onclick="setRating(5)" onmouseover="hoverRating(5)" onmouseout="resetHover()"></i>
            ...
            <div class="dialog-actions">
                <button type="button" onclick="submitRating()" class="btn btn-primary" data-i18n="submitRatingBtn">Submit Rating</button>
                <button type="button" onclick="closeRateModal()" class="btn btn-ghost" data-i18n="cancelBtn">Cancel</button>
            </div>
        </div>
    </div>

    <!-- Modal الستوري الفاخر -->
    ...
    <script>
        ...
        let selectedSpotifyId = "{{ song.spotify_id }}";
        ...
        function checkIfUserReviewed() {
            ...
            let userRev = reviews.find(r => r.username === currentUser);
            let btnText = document.getElementById('rateBtnText');
            if(userRev && btnText) {
                let lang = localStorage.getItem('musicy_lang') || 'en';
                btnText.innerText = (lang === 'ar') ? 'تعديل تقييمك ورأيك' : 'Edit Your Review';
                currentRatingVal = userRev.rating;
                setRating(currentRatingVal);
                if(userRev.comment) {
                    document.getElementById('reviewCommentInput').value = userRev.comment;
                }
            }
        }
        ...
        function submitRating() {
            ...
            .then(data => {
                if(data.success) {
                    document.getElementById('songScoreDisplay').innerText = data.new_rating;
        ...
        function generateStoryCanvas(data) {
            ...
            img.onload = function() {
                ...
                ctx.font = 'bold 54px "Bodoni Moda", Georgia, serif';
                ctx.textAlign = 'center';
                ctx.fillText(data.song_title, 540, 1220, 900);

                ctx.fillStyle = '#9ca3af';
                ctx.font = '38px sans-serif';
                ctx.fillText(data.artist, 540, 1290, 900);

                ctx.fillStyle = '#10b981';
                ctx.font = 'bold 64px "Bodoni Moda", Georgia, serif';
                ctx.fillText(`★ ${data.new_rating} / 5.0`, 540, 1430);

                ctx.fillStyle = '#6b7280';
                ctx.font = '28px sans-serif';
                ctx.fillText(`Rated by @${localStorage.getItem('songdb_user')} on Musicy`, 540, 1530);
            };
        }
        ...
        function downloadStory() {
            ...
            link.download = `Musicy-Story-${selectedSpotifyId}.png`;

...
<style>
    /* ==========================================================
       MUSICY — design system (Modified: Glow removed completely & color extraction improved)
       ========================================================== */
    :root {
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .sleeve .cover {
        ...
        background: var(--bg-3);
        box-shadow: 0 30px 60px -25px rgba(0, 0, 0, 0.75), 0 0 0 1px rgba(255, 255, 255, 0.1);
    }
    body.light-mode .sleeve .cover { box-shadow: 0 24px 48px -20px rgba(0, 0, 0, 0.15), 0 0 0 1px rgba(0, 0, 0, 0.1); }
    .vinyl-wrap {
    ...
    .tile-cover { aspect-ratio: 1; overflow: hidden; border-radius: 2px; background: var(--bg-3); box-shadow: 0 0 0 1px rgba(255, 255, 255, 0.1); }
    body.light-mode .tile-cover { box-shadow: 0 0 0 1px rgba(0, 0, 0, 0.1); }
    ...
    /* تم إزالة الـ Glow نهائياً من كفر الأغنية هنا وجعله ظلاً نظيفاً بدون توهج */
    ...
    .track-art .cover { box-shadow: 0 24px 48px -24px rgba(0, 0, 0, 0.7), 0 0 0 1px rgba(255, 255, 255, 0.1); transition: box-shadow 1.2s ease; }
    body.light-mode .track-art .cover { box-shadow: 0 24px 48px -24px rgba(0, 0, 0, 0.15), 0 0 0 1px rgba(0, 0, 0, 0.1); }
...
<div class="musical-bg-container" aria-hidden="true">
    ...
    <div class="musical-note" style="left: 18%; font-size: 18px; animation-duration: 15s; animation-delay: 2s;" data-speed="0.015"><i class="fa-solid fa-headphones"></i></div>
    <div class="musical-note" style="left: 35%; font-size: 28px; animation-duration: 10s; animation-delay: 1s;" data-speed="0.03"><i class="fa-solid fa-compact-disc"></i></div>
    <div class="musical-note" style="left: 52%; font-size: 20px; animation-duration: 14s; animation-delay: 3s;" data-speed="0.025"><i class="fa-solid fa-radio"></i></div>
    <div class="musical-note" style="left: 70%; font-size: 26px; animation-duration: 11s; animation-delay: 0.5s;" data-speed="0.02"><i class="fa-solid fa-microphone-lines"></i></div>
    <div class="musical-note" style="left: 88%; font-size: 22px; animation-duration: 13s; animation-delay: 2.5s;" data-speed="0.035"><i class="fa-solid fa-music"></i></div>
</div>
<script>
    ...
    function animateAmbient() {
        ...
        for (let n = 0; n < LINES; n++) {
            ...
            for (let x = 0; x <= W + step; x += step) {
                let nx = Math.min(x / W, 1);
                let env = Math.pow(Math.sin(Math.PI * nx), 1.15);
                let y = cy 
                    + Math.sin(nx * 5.2 + t * 0.32 + ph) * amp * 0.62 * env
                    + Math.sin(nx * 9.4 - t * 0.21 + ph * 1.8) * amp * 0.2 * env
                    + (u - 0.5) * amp * 1.05 * env * Math.sin(nx * 2.1 + t * 0.14);
                if (x === 0) { ctx.moveTo(x, y); } else { ctx.lineTo(x, y); }
            }
            let a = (0.07 + 0.26 * Math.sin(Math.PI * u)) * 0.5;
            ctx.strokeStyle = `rgba(var(--theme-color), ${a})`;
            ctx.stroke();
        }
    }
    animateAmbient();
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Bodoni+Moda:ital,opsz,wght@0,6..96,400..700;1,6..96,400..700&family=Hanken+Grotesk:wght@300;400;500;600;700&family=IBM+Plex+Sans+Arabic:wght@300;400;500;600;700&display=swap" rel="stylesheet">
...
<body class="musicy">
    ...
    <header class="top">
        <div class="top-inner">
            <div class="top-left">
                <a href="/" class="brand">
                    <svg class="brand-mark" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <circle class="ring-b" cx="16" cy="16" r="10" stroke="currentColor" stroke-width="1.5" stroke-dasharray="3 3"/>
                        <circle class="core" cx="16" cy="16" r="5"/>
                        <circle class="hole" cx="16" cy="16" r="2"/>
                    </svg>
            ...
            <div class="search">
                ...
                <input type="text" id="searchInput" class="search-input" data-i18n-placeholder="searchPlaceholder" placeholder="Search songs or artists on Spotify..." autocomplete="off">
                <div id="searchResults" class="results hidden"></div>
            </div>
            <div class="top-tools">
                <div class="tools">
                    <div class="lang">
                        <button class="lang-btn" data-l="en" onclick="setLanguage('en')">EN</button>
                        <button class="lang-btn" data-l="ar" onclick="setLanguage('ar')">عربي</button>
                    </div>
                    <button class="mode-btn" onclick="toggleDarkMode()" title="Toggle Dark/Light Mode">
                        <i id="darkModeIcon" class="fa-solid fa-moon text-theme"></i>
                        <span id="darkModeText" data-i18n="darkMode">Dark Mode</span>
                    </button>
                    <a href="/top-rated" class="icon-link" title="Top Rated"><i class="fa-solid fa-ranking-star"></i></a>
                </div>
    ...
    <main class="page">
        <section class="hero">
            <div class="hero-content">
                ...
                <p class="hero-desc" data-i18n="heroDesc">Search any song from Spotify to view ratings, read reviews, or publish your own rating with an ultra-luxurious story design.</p>
                <div class="hero-cta">
                    <a href="/top-rated" class="btn btn-primary"><i class="fa-solid fa-ranking-star"></i> <span data-i18n="topRated">Top Rated</span></a>
                </div>
            </div>
            {% if songs and songs|length > 0 %}
            <a href="/song/{{ songs[0].spotify_id }}" class="spotlight">
                <div class="sleeve">
                    <div class="vinyl-wrap">
                        <div class="vinyl"></div>
                    </div>
                    <img src="{{ songs[0].img }}" alt="{{ songs[0].title }}" class="cover">
                </div>
                <div class="spot-meta">
                    <div class="spot-title">{{ songs[0].title }}</div>
                    <div class="spot-artist">{{ songs[0].artist }}</div>
                    <div class="spot-rate">
                        <span class="stars" style="--r: {{ songs[0].rating }};"></span>
                        <b>{{ songs[0].rating }}</b>
                        <span class="sep"></span>
                        <span>{{ songs[0].votes }} <span data-i18n="votesText">votes</span></span>
                    </div>
                </div>
            </a>
            {% endif %}
        ...
        <section class="section">
            <div class="section-head">
                ...
                <button class="btn btn-ghost btn-sm" onclick="window.location.reload();"><i class="fa-solid fa-rotate"></i> <span data-i18n="refreshSongs">Refresh Songs</span></button>
            </div>
            <div class="wall">
                {% for s in songs %}
                <a href="/song/{{ s.spotify_id }}" class="tile">
                    <div class="tile-cover">
                        <img src="{{ s.img }}" alt="{{ s.title }}" loading="lazy">
                    </div>
                    <div class="tile-meta">
                        <h3 class="tile-title">{{ s.title }}</h3>
                        <p class="tile-artist">{{ s.artist }}</p>
                        <div class="tile-foot">
                            <span class="stars" style="--r: {{ s.rating }};"></span>
                            <b>{{ s.rating }}</b>
                            <span class="sep"></span>
                            <span class="votes-note">{{ s.votes }} <span data-i18n="votesText">votes</span></span>
                        </div>
                    </div>
                </a>
                {% endfor %}
    ...
    <footer class="foot">
        <div class="foot-inner">
            <div>&copy; 2026 <span class="foot-brand">Musicy</span>. All rights reserved.</div>
            <div>Crafted with <i class="fa-solid fa-heart"></i> for Music Lovers.</div>
        </div>
    </footer>

    <script>
        let currentUser = localStorage.getItem('musicy_user');
        let currentAvatar = localStorage.getItem('musicy_avatar') || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200';
        ...
        function updateUserNav() {
            ...
            if(currentUser) {
                ...
                area.innerHTML = `
                    <a href="/login" title="Login / Register"><i class="fa-solid fa-user"></i></a>
                `;
            }
        }
        ...
        function logout() {
            ...
            localStorage.removeItem('musicy_avatar');
            currentUser = null;
            updateUserNav();
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            searchTimeout = setTimeout(() => {
                ...
                .then(data => {
                    if(data.length === 0) {
                        searchResults.innerHTML = `<div style="padding: 14px; text-align: center; color: var(--muted); font-size: 13.5px;">No tracks found on Spotify.</div>`;
                    ...
                    data.forEach(track => {
                        html += `
                            <div onclick="window.location.href='/song/${track.spotify_id}'">
...
<body class="musicy">
    ...
    <main class="page narrow">
        <div class="page-head">
            <h1 class="h1" data-i18n="topRatedTitle">All Top Rated Songs</h1>
        ...
        <ul class="rank">
            ...
            <li class="rank-row {% if loop.index == 1 %}is-first{% endif %}">
                ...
                <img src="{{ s.img }}" alt="{{ s.title }}" class="rank-cover">
                <div class="rank-main">
                    <a href="/song/{{ s.spotify_id }}" class="rank-title">{{ s.title }}</a>
                    <div class="rank-sub">
                        <span>{{ s.artist }}</span>
                        <span class="sep"></span>
                ...
                <div class="rank-score">
                    <div class="score-num">{{ s.rating }}</div>
                    <span class="stars" style="--r: {{ s.rating }};"></span>
                    <span class="rank-votes"><b>{{ s.votes }}</b> <span data-i18n="votesText">votes</span></span>
                </div>
                <a href="/song/{{ s.spotify_id }}" class="chev"><i class="fa-solid fa-chevron-right"></i></a>
            </li>
            {% endfor %}
        </ul>
        {% else %}
...
<body class="musicy auth-page">
    ...
    <main class="auth-main">
        <div class="auth-art">
            <div class="sleeve">
                ...
                <img src="https://images.unsplash.com/photo-1514525253161-7a46d19cd819?auto=format&fit=crop&q=80&w=500" alt="Art" class="cover">
            </div>
        </div>
        <div class="auth-panel">
            <a href="/" class="back"><i class="fa-solid fa-arrow-left"></i> <span data-i18n="home">Home</span></a>
            <h1 class="auth-title" data-i18n="welcomeBack">Welcome Back</h1>
            ...
            <form id="loginForm" onsubmit="handleLogin(event)">
                <div class="field">
                    ...
                    <input type="email" id="email" class="input" required autocomplete="email" placeholder="name@example.com">
                </div>
                <div class="field">
                    <label data-i18n="passwordLabel">Password</label>
                    <input type="password" id="password" class="input" required autocomplete="current-password" placeholder="••••••••">
                </div>
                <div id="errorMsg" style="color: #ff2255; font-size: 13.5px; margin-top: 14px; display: none;"></div>
                <button type="submit" class="btn btn-primary btn-block" style="margin-top: 30px;"><span data-i18n="signInBtn">Sign In</span></button>
            </form>
    ...
    <script>
        function handleLogin(e) {
            ...
            .then(data => {
                if(data.success) {
                    localStorage.setItem('musicy_user', data.username);
                    localStorage.setItem('musicy_avatar', data.avatar);
    ...
    <main class="auth-main">
        <div class="auth-art">
            <div class="sleeve">
                ...
                <img src="https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?auto=format&fit=crop&q=80&w=500" alt="Art" class="cover">
            </div>
        </div>
        <div class="auth-panel">
            <a href="/" class="back"><i class="fa-solid fa-arrow-left"></i> <span data-i18n="home">Home</span></a>
            <h1 class="auth-title" data-i18n="createAccountTitle">Create Account</h1>
            ...
            <form id="registerForm" onsubmit="handleRegister(event)">
                <div class="field">
                    ...
                    <input type="text" id="username" class="input" required autocomplete="username" placeholder="your_name">
                </div>
                <div class="field">
                    <label data-i18n="emailLabel">Email Address</label>
                    <input type="email" id="email" class="input" required autocomplete="email" placeholder="name@example.com">
                ...
                <div class="field">
                    <label data-i18n="passwordLabel">Password</label>
                    <input type="password" id="password" class="input" required autocomplete="new-password" placeholder="••••••••">
                </div>
                <div id="errorMsg" style="color: #ff2255; font-size: 13.5px; margin-top: 14px; display: none;"></div>
                <button type="submit" class="btn btn-primary btn-block" style="margin-top: 30px;"><span data-i18n="createAccountLink">Create Account</span></button>
            </form>
...
<body class="musicy auth-page">
    ...
    <main class="auth-main" style="grid-template-columns: minmax(0, 1fr);">
        <div class="auth-panel" style="width: min(100%, 520px); text-align: center;">
            <a href="/" class="back" style="display: inline-flex;"><i class="fa-solid fa-arrow-left"></i> <span data-i18n="home">Home</span></a>
            <div>
                <img id="profileAvatar" src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200" alt="Avatar" class="avatar-lg">
            </div>
            <h2 id="profileUsername" class="who-line">Username</h2>
            <p class="lede" style="margin: 0 auto 24px;" data-i18n="profileTitle">Edit Profile & Avatar</p>
            
            <form id="avatarForm" onsubmit="handleAvatarUpload(event)" style="text-align: start;">
                <div class="field">
                    <label data-i18n="avatarFileLabel">Choose image from device (PC or Phone):</label>
                    <input type="file" id="avatarFile" class="file" accept="image/*" required>
                </div>
                <div id="errorMsg" style="color: #ff2255; font-size: 13.5px; margin-top: 14px; display: none;"></div>
                <button type="submit" class="btn btn-primary btn-block" style="margin-top: 26px;"><span data-i18n="saveAvatarBtn">Upload & Save Avatar</span></button>
            </form>
        </div>
    </main>
    ...
    <script>
        ...
        if(!currentUser) {
            ...
            let av = localStorage.getItem('musicy_avatar');
            if(av) {
        ...
        function handleAvatarUpload(e) {
            ...
            .then(data => {
                if(data.success) {
                    ...
                    err.innerText = data.message || 'Upload failed';
                    err.style.display = 'block';
                }
            }).catch(err => console.error(err));
        }
...
<body class="musicy">
        ...
        <section class="track">
            <div class="track-art">
                <div class="sleeve">
                    ...
                    <img id="songCoverImg" src="{{ song.img }}" alt="{{ song.title }}" class="cover" crossorigin="anonymous">
                </div>
            </div>
            <div class="track-info">
                ...
                <div class="track-rating">
                    <div class="big-score" id="songRatingDisplay">{{ song.rating }}</div>
                    <div>
                        <span class="stars lg" style="--r: {{ song.rating }};"></span>
                        <div class="votes"><b id="songVotesDisplay">{{ song.votes }}</b> <span data-i18n="votesText">votes</span></div>
                    </div>
                </div>
                <div class="actions">
                    {% if song.preview_url %}
                    <audio id="audioPreview" src="{{ song.preview_url }}" preload="none"></audio>
                    <button class="btn btn-ghost" onclick="togglePlayPreview()"><i id="playIcon" class="fa-solid fa-play"></i> <span id="playText">Preview Audio</span></button>
                    {% endif %}
                    ...
                    {% endif %}
                    <button class="btn btn-ghost" onclick="openRateModal()"><i class="fa-solid fa-star"></i> <span id="rateBtnText" data-i18n="rateSongBtn">Rate This Song</span></button>
                </div>
            </div>
        </section>
            ...
            <div class="cols">
                <div class="reviews-col">
                    <div class="section-head" style="margin-bottom: 24px;">
                        <h2 class="h2"><span data-i18n="reviewsTitle">Reviews & Ratings</span> <span class="count" id="reviewsCount">({{ reviews|length }})</span></h2>
                    </div>
                    <div id="reviewsList">
                        {% if reviews and reviews|length > 0 %}
                            ...
                            <div class="review" data-review-id="{{ r.id }}">
                                <div class="review-head">
                                    <img src="{{ r.avatar }}" alt="{{ r.username }}" class="avatar">
                                    <div class="who">
                                        <h4>{{ r.username }}</h4>
                                ...
                                <div class="review-foot">
                                    <button class="like-btn" onclick="toggleLike({{ r.id }})">
                                        <i class="fa-solid fa-heart text-gray-400" id="likeIcon_{{ r.id }}"></i>
                                        <b id="likeCount_{{ r.id }}">{{ r.likes }}</b>
                                    </button>
                                </div>
                            </div>
                            {% endfor %}
                ...
                <div class="ledger">
                    ...
                    <dl>
                        <div class="row">
                            ...
                            <dd class="accent" id="ledgerRating">{{ song.rating }} / 5.0</dd>
                        </div>
                        <div class="row">
                        ...
                        <div class="row">
                            <dt>Spotify ID</dt>
                            <dd style="font-size: 12px; font-family: monospace;">{{ song.spotify_id }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
        ...
        <div class="dialog">
            ...
            <div class="field" style="margin-top: 20px;">
                ...
                <div id="starContainer">
                    <i class="fa-solid fa-star" data-val="1" onclick="setRatingVal(1)" onmouseover="hoverRatingVal(1)" onmouseout="resetRatingHover()"></i>
                    <i class="fa-solid fa-star" data-val="2" onclick="setRatingVal(2)" onmouseover="hoverRatingVal(2)" onmouseout="resetRatingHover()"></i>
                    <i class="fa-solid fa-star" data-val="3" onclick="setRatingVal(3)" onmouseover="hoverRatingVal(3)" onmouseout="resetRatingHover()"></i>
                    <i class="fa-solid fa-star" data-val="4" onclick="setRatingVal(4)" onmouseover="hoverRatingVal(4)" onmouseout="resetRatingHover()"></i>
                    <i class="fa-solid fa-star" data-val="5" onclick="setRatingVal(5)" onmouseover="hoverRatingVal(5)" onmouseout="resetRatingHover()"></i>
                    <span id="starValueText">5.0</span>
                </div>
            </div>
            <div class="field">
                <label data-i18n="commentLabel">Your Review / Comment:</label>
                <textarea id="reviewComment" class="textarea" rows="4" placeholder="Write your thoughts about this masterpiece..."></textarea>
            </div>
            <div id="modalError" style="color: #ff2255; font-size: 13.5px; margin-top: 10px; display: none;"></div>
            <div class="dialog-actions">
                <button class="btn btn-primary" onclick="submitRating()"><span data-i18n="submitRatingBtn">Submit Rating & Generate Story</span></button>
                <button class="btn btn-ghost" onclick="closeRateModal()"><span data-i18n="cancelBtn">Cancel</span></button>
            </div>
        </div>
    </div>

    <!-- Modal Luxury Story Preview -->
    <div id="storyModal" class="modal hidden">
        <div class="dialog story">
            ...
            <div class="dialog-actions">
                <button class="btn btn-primary" onclick="downloadStory()"><i class="fa-solid fa-download"></i> <span data-i18n="downloadStoryBtn">Download Luxury Story</span></button>
                <button class="btn btn-ghost" onclick="closeStoryModal()"><span data-i18n="closeStoryBtn">Close & Continue</span></button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // تحسين جودة التقاط لون الأغنية بدقة فائقة وعالية الجودة لتغيير الثيم بدقة
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (imgEl) {
                ...
                img.onload = function() {
                    ...
                    try {
                        ...
                        let bestR = 16, bestG = 185, bestB = 129;
                        let maxScore = -1;
                        ...
                        for (let i = 0; i < imageData.length; i += 16) {
                            ...
                            // تصفية الألوان السوداء الصرفة والبيضاء/الرمادية الشاحبة لالتقاط لون حيوي ونقي
                            if (lightness > 22 && lightness < 232 && saturation > 0.18) {
                        ...
                        document.documentElement.style.setProperty('--theme-color', `${bestR}, ${bestG}, ${bestB}`);
                    } catch(e) {
        ...
        function setRatingVal(v) {
            selectedStars = v;
            document.getElementById('starValueText').innerText = v.toFixed(1);
            highlightStars(v);
        }

        function hoverRatingVal(v) {
            highlightStars(v);
        }
        ...
        function submitRating() {
            ...
            .then(data => {
                if(data.success) {
                    document.getElementById('songRatingDisplay').innerText = data.new_rating;
                    document.getElementById('songVotesDisplay').innerText = data.votes;
                    document.getElementById('ledgerRating').innerText = data.new_rating + ' / 5.0';
                    ...
                    err.innerText = data.message || 'Error submitting rating';
                    err.style.display = 'block';
                }
            }).catch(err => console.error(err));
        }
        ...
        function generateStoryCanvas(data) {
            ...
            img.onload = function() {
                ...
                let title = data.song_title || '{{ song.title }}';
                if(title.length > 26) title = title.substring(0, 24) + '...';
                ctx.fillText(title, 540, 1160);

                ctx.fillStyle = '#9ca3af';
                ctx.font = '400 36px sans-serif';
                ctx.fillText(data.artist || '{{ song.artist }}', 540, 1230);

                ctx.fillStyle = '#10b981';
                ctx.font = '700 72px "Bodoni Moda", serif';
                ctx.fillText(`★ ${selectedStars}.0`, 540, 1370);

                ctx.fillStyle = '#6b7280';
                ctx.font = '500 28px sans-serif';
                ctx.fillText(`Reviewed by @${currentUser} on Musicy`, 540, 1450);

                ctx.strokeStyle = 'rgba(255, 255, 255, 0.15)';
                ctx.lineWidth = 2;
                ctx.beginPath();
                ctx.arc(540, 1680, 70, 0, Math.PI * 2);
                ctx.stroke();

                ctx.fillStyle = '#f9fafb';
                ctx.font = '600 24px sans-serif';
                ctx.fillText('MUSICY', 540, 1688);
            };
        }
        ...
        function toggleLike(reviewId) {
            ...
            .then(data => {
                if(data.success) {
                    let countEl = document.getElementById(`likeCount_${reviewId}`);
                    let iconEl = document.getElementById(`likeIcon_${reviewId}`);
                    countEl.innerText = data.likes;
                    if(data.liked) {
                        iconEl.className = 'fa-solid fa-heart liked-red';
                        iconEl.classList.add('like-animate');
                        setTimeout(() => iconEl.classList.remove('like-animate'), 400);
                    } else {
                        iconEl.className = 'fa-solid fa-heart text-gray-400';
                    }
                }
            }).catch(err => console.error(err));
        }

        function fetchLikesStatus() {
            ...
            .then(likedDict => {
                for(let rid in likedDict) {
                    if(likedDict[rid]) {
                        let iconEl = document.getElementById(`likeIcon_${rid}`);
                        if(iconEl) iconEl.className = 'fa-solid fa-heart liked-red';
                    }
                }
            }).catch(err => console.error(err));
        }

...
<style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    /* تم حذف تأثير التوهج الضوئي من هنا وتعديله ليكون ظلاً محايداً */
    .track-art .cover { box-shadow: 0 40px 70px -34px rgba(0, 0, 0, 0.85), 0 0 0 1px rgba(var(--theme-color), 0.15); transition: box-shadow 1.2s ease; }
    body.light-mode .track-art .cover { box-shadow: 0 34px 60px -30px rgba(var(--theme-color), 0.25), 0 0 0 1px rgba(var(--theme-color), 0.15); }
...
<body class="musicy auth-page">
    ...
    <main class="auth-main">
        <div class="auth-art">
            <div class="sleeve">
                <div class="vinyl-wrap"><div class="vinyl"></div></div>
                <img id="profileArtImg" class="cover" src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=500" alt="Avatar">
            </div>
        </div>
        <div class="auth-panel text-center">
            <a href="/" class="back" style="float: inline-start;"><i class="fa-solid fa-chevron-left"></i><span data-i18n="home">Home</span></a>
            <div style="clear: both;"></div>
            <img id="profileAvatarDisplay" src="https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=200" class="avatar-lg mx-auto">
            <h1 class="who-line" id="profileUsernameDisplay">User</h1>
            <p class="lede mx-auto" data-i18n="profileTitle" style="margin-bottom: 24px;">Edit Profile &amp; Avatar</p>

            <form id="avatarFileForm" onsubmit="handleAvatarFileUpload(event)">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if(avatar) {
                ...
                document.getElementById('profileArtImg').src = avatar;
            }
        });
        ...
        function handleAvatarFileUpload(e) {
            ...
            .then(data => {
                if(data.success) {
                    localStorage.setItem('songdb_avatar', data.avatar);
                    document.getElementById('profileAvatarDisplay').src = data.avatar;
                    document.getElementById('profileArtImg').src = data.avatar;
...
<body class="musicy">
        ...
        <div class="track">
            ...
            <div class="track-info">
                ...
                <div class="track-rating">
                    <div class="big-score" id="songRatingScore">{{ song.rating }}</div>
                    <div>
                ...
                <div class="actions">
                    ...
                    <button type="button" onclick="togglePlay()" id="playBtn" class="btn btn-ghost"><i class="fa-solid fa-play" id="playIcon"></i><span>Preview</span></button>
                    {% endif %}
        ...
        <div class="section cols">
            <div class="reviews-col">
                <div class="section-head">
                    ...
                    <button type="button" onclick="openRateModal()" class="btn btn-primary btn-sm"><i class="fa-solid fa-pen"></i><span data-i18n="writeReviewBtn">Write Review</span></button>
                </div>
                ...
                <div id="reviewsListContainer">
                    ...
                    {% endfor %}
                    {% else %}
                    <div class="empty" id="noReviewsMessage" data-i18n="noReviews">No reviews yet. Be the first to review this song!</div>
                    {% endif %}
                </div>
            </div>
        ...
        <div class="dialog">
            ...
            <div class="field">
                ...
                <div id="starContainer">
                    <i class="fa-solid fa-star" data-val="1" onclick="setRatingVal(1)"></i>
                    <i class="fa-solid fa-star" data-val="2" onclick="setRatingVal(2)"></i>
                    <i class="fa-solid fa-star" data-val="3" onclick="setRatingVal(3)"></i>
                    <i class="fa-solid fa-star" data-val="4" onclick="setRatingVal(4)"></i>
                    <i class="fa-solid fa-star" data-val="5" onclick="setRatingVal(5)"></i>
                    <span id="starValueText">5</span>
                </div>
            </div>
            <div class="field">
                <label data-i18n="commentLabel">Your Review / Comment:</label>
                <textarea id="commentInput" rows="4" class="textarea" placeholder="Write your thoughts..."></textarea>
            </div>
    ...
    <script>
        ...
        function setRatingVal(val) {
            ...
            stars.forEach(s => {
                let v = parseInt(s.getAttribute('data-val'));
                if(v <= val) {
        ...
        function submitRating() {
            ...
            .then(data => {
                if(data.success) {
                    ...
                    let btnText = document.querySelector('#rateModalBtnText span');
                    if(btnText) {
                        let lang = localStorage.getItem('musicy_lang') || 'en';
        ...
        function checkUserLikes() {
            ...
            let ids = [];
            reviewEls.forEach(el => ids.push(parseInt(el.getAttribute('data-review-id'))));
            if(ids.length === 0) return;

            fetch('/api/check_likes', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({username: user, review_ids: ids})
            })
            .then(res => res.json())
            .then(likedDict => {

...
def spotify_callback():
  ...
  try:
    ...
    pass
  return redirect(url_for('top_rated_page'))
...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Amiri:ital,wght@0,400;0,700;1,400&amp;family=Bodoni+Moda:ital,opsz,wght@0,6..96,400..700;1,6..96,400..700&amp;family=Hanken+Grotesk:wght@300..700&amp;family=IBM+Plex+Sans+Arabic:wght@300;400;500;600&amp;display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <!-- تضمين مكتبة Spotify Web Playback SDK -->
...
<body class="musicy">
        ...
        <section class="track">
            ...
            <div class="track-info">
                ...
                <div class="actions">
                    ...
                    <button type="button" id="sdkPlayBtn" onclick="playFullSong('spotify:track:{{ song.spotify_id }}')" class="btn" style="background: linear-gradient(135deg, #1DB954 0%, #1ed760 100%); color: #030712; border: none; box-shadow: 0 10px 25px -5px rgba(29, 185, 84, 0.4); font-weight: 600;">
                        <i class="fa-solid fa-play"></i>
                        <span>تشغيل كامل (SDK)</span>
                    </button>
                    {% else %}
                    <a href="/spotify/login" class="btn" style="background: linear-gradient(135deg, #1DB954 0%, #1ed760 100%); color: #030712; border: none; box-shadow: 0 10px 25px -5px rgba(29, 185, 84, 0.4); font-weight: 600;">
                        <i class="fa-brands fa-spotify"></i>
                        <span>ربط Spotify للتشغيل الكامل</span>
                    </a>
                    {% endif %}
    ...
    <script>
        ...
        const spotifyToken = "{{ session.get('spotify_access_token', '') }}";

        window.onSpotifyWebPlaybackSDKReady = () => {
            if (!spotifyToken) return;
            spotifyPlayer = new Spotify.Player({
                name: 'Musicy Web Player',
            ...
            spotifyPlayer.addListener('account_error', ({ message }) => {
                console.error('Account error: Requires Spotify Premium', message);
                let btn = document.getElementById('sdkPlayBtn');
                if(btn) btn.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i><span>يتطلب حساب Premium</span>';
            });

            spotifyPlayer.connect();
        };

        function playFullSong(spotifyUri) {
            if (!spotifyToken) {
                window.location.href = '/spotify/login';
                return;
            }
            if (!spotifyDeviceId) {
                alert('جاري تهيئة مشغل Spotify، يرجى التأكد من تشغيل تطبيق سبوتيفاي على جهازك وأن حسابك Premium.');
                return;
            }
            ...
            fetch(`https://api.spotify.com/v1/me/player/play?device_id=${spotifyDeviceId}`, {
                ...
                if (!res.ok) {
                    res.json().then(data => {
                        console.error(data);
                        alert('تعذر تشغيل الأغنية. تأكد من فتح تطبيق سبوتيفاي وأنك تمتلك حساب Premium.');
                    });
                } else {
                    let playBtn = document.getElementById('sdkPlayBtn');
                    if(playBtn) {
                        playBtn.innerHTML = '<i class="fa-solid fa-pause"></i><span>جاري التشغيل الآن</span>';
                    }
                }
            }).catch(err => {
                console.error(err);
